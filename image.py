...
st.markdown("""
    ...
    /* 2. [수정 핵심] 사진 움직임 방지 및 이미지 카드 최적화 */
    .image-card-container {
        ...
        /* 내부 요소가 변해도 컨테이너 크기가 변하지 않도록 설정 */
        overflow: hidden; 
    }
    
    /* 이미지 영역 높이를 고정하여 체크 시 덜컥거림 방지 */
    .stImage {
    ...
    .selected-img img {
        ...
        transition: filter 0.2s ease-in-out; /* 부드러운 전환 */
    }

    /* 3. 파란색 박스 버튼 UI (기존 그대로 유지) */
    .stButton > button {
...
with st.container():
    ...
    if st.session_state['results']:
        ...
        if temp_sel:
            ...
            inf3.download_button(f"📥 {len(temp_sel)}장 다운로드", zip_buf.getvalue(), "images.zip", use_container_width=True, key="top_dl")
...
if st.session_state['results']:
    ...
    for idx, img in enumerate(st.session_state['results']):
        with cols[idx % 3]:
            ...
            # 이미지 클래스 분기 (체크 여부에 따라 블러 효과만 부여)
    ...
    if temp_sel:
        st.markdown("---")
        st.download_button(f"📥 선택한 {len(temp_sel)}장 최종 다운로드", zip_buf.getvalue(), "final.zip", use_container_width=True, key="btm_dl")
